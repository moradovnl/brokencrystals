...
export class FileController {
  ...
  private isValidPath(filePath: string): boolean {
    ...
    try {
      ...
      // Additional check to ensure the path is within a specific directory
      const basePath = path.resolve('config/products/crystals');
      const resolvedPath = path.resolve(filePath);
      return resolvedPath.startsWith(basePath); // Ensure the path is within the allowed directory
    }
  }
}
